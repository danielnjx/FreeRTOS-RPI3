...
typedef struct tskTaskControlBlock
{
	...
	// Daniel
	unsigned portBASE_TYPE preempt;
 ...
 */
static void prvInitialiseTCBVariables( tskTCB *pxTCB, const signed char * const pcName, unsigned portBASE_TYPE uxPriority, const xMemoryRegion * const xRegions, unsigned short usStackDepth, unsigned portBASE_TYPE preempt ) PRIVILEGED_FUNCTION;
...
/*lint +e956 */
// Daniel New functions for preemption change

void makePreemptive()
{
	taskENTER_CRITICAL();
	pxCurrentTCB->preempt = 1;
	taskEXIT_CRITICAL();
}

void makeUnpreemptive()
{
	taskENTER_CRITICAL();
	pxCurrentTCB->preempt = 0;
	taskEXIT_CRITICAL();
}
...
signed portBASE_TYPE xTaskGenericCreate( pdTASK_CODE pxTaskCode, const signed char * const pcName, unsigned short usStackDepth, void *pvParameters, unsigned portBASE_TYPE uxPriority, xTaskHandle *pxCreatedTask, unsigned portBASE_TYPE preempt, portSTACK_TYPE *puxStackBuffer, const xMemoryRegion * const xRegions )
{
...
tskTCB * pxNewTCB;
	...
	if( pxNewTCB != NULL )
	{
		...
		prvInitialiseTCBVariables( pxNewTCB, pcName, uxPriority, xRegions, usStackDepth, preempt);
	...
	if( xReturn == pdPASS )
	{
		if( xSchedulerRunning != pdFALSE )
		{
			...
			//Daniel - And is allowed to be preempted
			if( pxCurrentTCB->uxPriority < uxPriority && pxCurrentTCB->preempt == 1)
			//if( pxCurrentTCB->uxPriority < uxPriority)
			{
		...
		if( ( pxTCB != NULL ) && ( pxTCB != pxCurrentTCB ) )
		{
			taskENTER_CRITICAL();
			{
				if( xTaskIsTaskSuspended( pxTCB ) == pdTRUE )
				{
					...
					// Daniel -- Check if the currrent task can be preempted..
					if( pxTCB->uxPriority >= pxCurrentTCB->uxPriority && pxCurrentTCB->preempt == 1)
...
portBASE_TYPE xReturn;
	...
	#else
	{
		...
		xReturn = xTaskCreate( prvIdleTask, ( signed char * ) "IDLE", tskIDLE_STACK_SIZE, ( void * ) NULL, ( tskIDLE_PRIORITY | portPRIVILEGE_BIT ), NULL, 1 );
	}
	#endif	
...
{
	//Daniel
	//println("ContextSwitch",RED_TEXT);
	if( uxSchedulerSuspended != ( unsigned portBASE_TYPE ) pdFALSE )
	{
	...
	else
	{
		...
		//Daniel get next owner only if preemption is allowed in current tcb
		if (pxCurrentTCB->preempt == 1)
		{
			listGET_OWNER_OF_NEXT_ENTRY( pxCurrentTCB, &( pxReadyTasksLists[ uxTopReadyPriority ] ) );
		}//else, do not get next task in the list
...
// Daniel
__attribute__((no_instrument_function))
static void prvInitialiseTCBVariables( tskTCB *pxTCB, const signed char * const pcName, unsigned portBASE_TYPE uxPriority, const xMemoryRegion * const xRegions, unsigned short usStackDepth, unsigned portBASE_TYPE preempt)
{
